...
class MainActivity : AppCompatActivity() {
    ...
    private fun showHome(){
        ...
        val play=action("Jogar",text,accent,R.drawable.ic_play,22)
        play.alpha=0f
        play.scaleX=.86f
        play.scaleY=.86f
    ...
    private fun showMain(){
        ...
        listOf(4,6,8,10,12,20,100).forEach{d->
            ...
            diceGrid.addView(b,GridLayout.LayoutParams().apply{width=0;height=dp(48);columnSpec=GridLayout.spec(GridLayout.UNDEFINED,1f);setMargins(dp(3),dp(3),dp(3),dp(3))})
        ...
        modes.addOnButtonCheckedListener{_,id,checked->if(checked){mode=if(id==two.id)VisualMode.TWO_D else VisualMode.THREE_D;updateVisual()}}
        val destiny=card()
    ...
    private fun rollAll(){
        ...
        p.results.clear()
        repeat(diceCount){p.results.add(DiceEngine.roll(sides,p.luck,p.curse,p.antiJinx,p.percentageMode,p.luckPercent,p.cursePercent))}
        if(mode==VisualMode.TWO_D) visual2d.roll(sides,p.results.toIntArray()) else visual3d.roll()
        renderResults()
    }
    ...
    private fun renderResults(){
        ...
        p.results.forEachIndexed{i,v->
            val row=LinearLayout(this).apply{gravity=Gravity.CENTER_VERTICAL;background=rounded(panel2,18);setPadding(dp(14),dp(8),dp(8),dp(8))}
    ...
    private fun showSettings(){
        ...
        val anti=Switch(this).apply{
            text="Anti-zikamento nesta rodada"
            isChecked=p.antiJinx
            setTextColor(this@MainActivity.text)
        }
        box.addView(anti)
        val percent=Switch(this).apply{
            text="Sorte e azar em porcentagem"
            isChecked=p.percentageMode
            setTextColor(this@MainActivity.text)
        }
        box.addView(percent)
        ...
        box.addView(luck)
        box.addView(curse)
        percent.setOnCheckedChangeListener{_,c->
            luck.hint=if(c)"Sorte (%)" else "Sorte (pontos)"
            curse.hint=if(c)"Azar (%)" else "Azar (pontos)"
        }
        ...
        dialog.setOnShowListener{
            dialog.getButton(AlertDialog.BUTTON_POSITIVE).setOnClickListener{
                ...
                if(p.percentageMode){
                    p.luckPercent=a.coerceIn(0,100)
                    p.cursePercent=c.coerceIn(0,100)
                }else{
                    p.luck=a.coerceIn(0,100)
                    p.curse=c.coerceIn(0,100)
                }
                renderPlayers()
                renderResults()
                dialog.dismiss()
        ...
        if(players.size>1){
            dialog.setOnShowListener{dlg->
                dialog.getButton(AlertDialog.BUTTON_POSITIVE).setOnClickListener{
                    ...
                    val a=luck.text.toString().toIntOrNull()?:0
                    val c=curse.text.toString().toIntOrNull()?:0
                    if(p.percentageMode){p.luckPercent=a.coerceIn(0,100);p.cursePercent=c.coerceIn(0,100)}else{p.luck=a.coerceIn(0,100);p.curse=c.coerceIn(0,100)}
    ...
    private fun showDonation(){
        val copy=action("Copiar mensagem",text,panel2,R.drawable.ic_favorite,18)
        ...
        box.addView(label("Se quiser apoiar o projeto, copie esta mensagem e combine a doação com o desenvolvedor.",14f,muted,false),margin(0,8,0,12))
        box.addView(copy)
    ...
    private fun showHelp(){
        AlertDialog.Builder(this).setTitle("Como usar").setMessage("Escolha o player, o dado e a quantidade. 2D mostra dados visuais e anima os números até o resultado. 3D usa OpenGL ES nativo e permite vários dados. Sorte/azar é aplicado dentro do limite do dado: D20 nunca passa de 20. Nova rodada desliga o anti-zikamento.").setPositiveButton("Entendi",null).show()
    }
    ...
    private fun icon(desc:String,res:Int,bgColor:Int)=action("",text,bgColor,res,16).apply{
    ...
    private fun number(h:String,v:Int)=EditText(this).apply{
        hint=h
        setText(v.toString())
        setTextColor(this@MainActivity.text)
        setHintTextColor(muted)
